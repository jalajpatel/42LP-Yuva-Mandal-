...
import WelcomeImage from "@/components/Welcome"; // Import your WelcomeImage component
...
export default function Home() {
  ...
  const [showWelcome, setShowWelcome] = React.useState(true); // State to control the visibility of the welcome image

  React.useEffect(() => {
    const timer = setTimeout(() => {
      setShowWelcome(false); // Hide the welcome image after a certain time (e.g., 3 seconds)
    ...
    >
      <AnimatePresence>
        ...
        <div className="absolute z-20 h-full w-full">
          <Header />
          {showWelcome && <WelcomeImage />} {/* Display the welcome image */}
          <div className={`flex h-full w-full flex-col md:flex-row ${showWelcome ? 'hidden' : ''}`}> {/* Flex column for mobile */}
            <div className="flex h-full flex-1 flex-col justify-end px-5 md:mb-0 md:justify-center md:px-10">
              <SlideInfo
                transitionData={transitionData}
                currentSlideData={currentSlideData}
              />
