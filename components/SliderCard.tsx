...
function SliderCard({ data }: Props) {
  ...
  return (
    <>
      {/* Render the card only if it's not a mobile device
      {!isMobile && (
        <motion.div
          ...
          style={{
            backgroundColor: "rgba(255, 255, 255, 0.1)", // Adjust background color and opacity
          }}
        >
          <motion.img
            layoutId={data.img}
            ...
            src={data.img}
            className="absolute inset-0 rounded-2xl object-cover brightness-75"
          />
